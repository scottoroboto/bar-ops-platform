...
// Self-service password change — reached only through requireSession('full'),
// which already means this request just proved the current password via a
// full login or a step-up re-check. No separate "current password" field
// needed on top of that; see public/change-password.html.
async function setPassword({ personId, password }) {
const passwordHash = await bcrypt.hash(password, 10);
return withServiceClient(async (client) => {
await client.query('UPDATE people SET password_hash = $1 WHERE id = $2', [passwordHash, personId]);
return { ok: true };
});
}
