// Manage Access — Inventory Control. Owner-only, both by server-side gate
// (every /api/inventory/access/* route 403s anyone else) and by this page
// immediately bouncing a non-owner back out. Direct port of
// public/cash-access.js — same three sections (Position Defaults, People,
// Change Log), pointed at /api/inventory/access/* and this app's own four
// tier names. See the implementation plan's design call (c): this stays
// owner-only even for someone at Full authority tier, the same hard rule
// Cash Handling uses, not a literal reading of "Full authority manages
// access" — confirmed with Scotto rather than assumed.
let ME = null;
let TIER_LABELS = { full_authority: 'Full authority', lead: 'Lead', counter: 'Counter', no_access: 'No access' };
let ALL_PEOPLE = [];
...
async function loadDefaults() {
  ...
  try {
    const result = await api('/api/inventory/access/defaults');
...
async function saveDefault(positionId) {
  ...
  try {
    await withStepUp(() => api('/api/inventory/access/defaults', { method: 'POST', body: { positionId, tier } }));
...
async function loadPeople() {
  ...
  el.innerHTML = '<p class="muted">Loading…</p>';
  try {
    const result = await api('/api/inventory/access/overrides');
    ...
    return `
      <div class="list-row">
        <div>
          ...
          <div class="sub">${escapeHtml(p.position || '—')} · ${escapeHtml(p.location_name || 'no location')}${!p.app_enabled ? ' · Inventory Control off' : ''}</div>
        </div>
...
function editPersonRowHtml(p) {
  return `
    <div class="list-row" style="flex-direction:column; align-items:stretch;">
      ...
      <input id="editPersonNote" placeholder="Why this override — e.g. filling in until we hire a lead">
...
async function saveEditPerson(personId) {
  ...
  try {
    await withStepUp(() => api('/api/inventory/access/overrides', { method: 'POST', body: { personId, tier, note: note || undefined } }));
...
async function revertPerson(personId) {
  try {
    await withStepUp(() => api(`/api/inventory/access/overrides/${personId}/revert`, { method: 'POST' }));
...
async function loadLog() {
  ...
  el.innerHTML = '<p class="muted">Loading…</p>';
  try {
    const result = await api('/api/inventory/access/log');
...
(async function init() {
  ...
  if (ME.role !== 'owner') {
    document.getElementById('panelMain').innerHTML =
      '<div class="card"><p>Only the owner can manage Inventory Control access.</p><p><a href="/inventory.html">Back to Inventory</a></p></div>';
    return;
  }
  document.getElementById('panelMain').innerHTML = `
    <h1 class="page-title">Manage Access</h1>
    <p class="muted" style="margin-top:-6px;">Only the owner can grant or change Inventory Control access — this never delegates to managers, even at Full authority.</p>
