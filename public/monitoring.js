...
async function runUnifiProbe() {
  ...
  try {
    ...
    el.innerHTML = `
      ...
      ${p.devices.length ? p.devices.map(d => `<div class="list-row"><div><div class="name">${escapeHtml(d.name || '(unnamed)')} <span class="badge ${d.raw === 'online' ? 'on' : d.raw === 'offline' ? 'danger' : 'off'}">${escapeHtml(d.status || '?')}</span></div><div class="sub">${d.hostName ? 'on ' + escapeHtml(d.hostName) + ' · ' : ''}${escapeHtml(d.model || '?')} · ${escapeHtml(kindLabel[d.kind] || d.kind)} · mac <code>${escapeHtml(d.mac || '?')}</code>${d.ip ? ' · ' + escapeHtml(d.ip) : ''}</div></div>
      ...
      ${(p.ispAttempts || []).length && !p.isp.length ? `<div class="sub" style="word-break:break-all;">${p.ispAttempts.map(a => escapeHtml(`${a.url} → ${a.error ? 'error: ' + a.error : a.items + ' item(s): ' + a.preview}`)).join('<br>')}</div>` : ''}
